...
function BaseEffect:OnStartServer(...)
	--print(`BaseEffect:OnStartServer() called for {self}`)
end

function BaseEffect:GetPlayersOtherThan(PlayerToExclude)
	local Players = game.Players:GetPlayers()
	local PlayersOtherThan = {}
	for _,Player in pairs(Players) do
		if Player ~= PlayerToExclude then
			table.insert(PlayersOtherThan,Player)
		end
	end
	return PlayersOtherThan
end
...
function BaseEffect:Destroy()
	...
	if self._executionThread and coroutine.status(self._executionThread) ~= "dead" then
		...
		--No need for this error... It's doing what it's supposed to do.
