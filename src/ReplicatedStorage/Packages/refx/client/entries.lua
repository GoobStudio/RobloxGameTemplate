...
function entries.processLocalEntry(effect)
	...
	effect:OnConstruct(table.unpack(effect.Configuration))
	effect:_start()
end
...
-- Introspection: snapshot of every effect currently tracked on this client.
-- Used by debug/admin tooling. Purely read-only; returns a fresh array.
function entries.getActiveEntries()
	local result = {}
	for id, entry in entriesMap do
		local effect = entry.Effect
		local lifetime = tick() - entry.StartTimestamp
		table.insert(result, {
			Id = id,
			Name = tostring(getmetatable(effect)),
			Lifetime = lifetime,
			MaxLifetime = effect.MaxLifetime,
			IsOverLifetime = lifetime > effect.MaxLifetime,
		})
	end
	return result
end
