...
local serverEntries = require(script.Parent.serverEntries)
...
return function(ctorName, ...)
	...
	-- Pull the ctor args in the same order the client receives them (i.e. the "..." passed to this proxy factory)
	local function getCtorArgs()
		-- params[0] is a sentinel; real args start at 1
		return table.unpack(params, 1, #params)
	end
	...
	local reservedFunctions = {
		Start = function(...)
			...
			serverEntries.register(id, ctorName)
			remotes.__refx_create:firePlayers(players, ctorName, id, params)

			-- IMPORTANT:
			-- Call server start with the SAME ctor args the client constructor gets.
			-- (Not the players array, not a wrapped table)
			proxy:OnStartServer(getCtorArgs())

			return proxy
		end,

		OnStartServer = function(...)
			logger.assert(initialized, "Cannot :OnStartServer() before effect has started.")

			-- Call the effect's server start with proxy as self + ctor args
			local _ = omitSelf(...) -- discard any accidental extra args from a manual call
			if constructor.OnStartServer ~= nil then
				constructor.OnStartServer(proxy, getCtorArgs())
			end
		end,

		WithinRange = function(proxy, pos, radius)
		...
		Destroy = function()
			...
			destroyed = true
			serverEntries.unregister(id)
			remotes.__refx_destroy:firePlayers(players, id)
			return proxy
		end,

		GetPlayersInRange = function(position, radius)
			local allPlayers = game.Players:GetPlayers()
			local playersInRange = {}
			for _, player in pairs(allPlayers) do
				if player.Character and player.Character.PrimaryPart then
